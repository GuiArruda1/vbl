  ...
  <footer class="w-full bg-white font-body pt-16 lg:pt-24 pb-8">
    <div class="max-w-[1920px] mx-auto px-6 xl:px-[8.33%]">
      ...
      <div class="flex flex-col lg:flex-row lg:justify-between items-start gap-12 lg:gap-6 mb-16 xl:mb-24">
        ...
        <!-- Logo -->
        <div class="flex items-center gap-6 lg:gap-[38px] lg:w-auto flex-shrink-0">
          <?php 
          $footer_logo = get_theme_mod( 'vbl_footer_logo' );
          if ( $footer_logo ) : ?>
              <img src="<?php echo esc_url( $footer_logo ); ?>" alt="<?php bloginfo( 'name' ); ?>" class="h-12 lg:h-[60px] w-auto">
          <?php else : ?>
              <img src="<?php echo vbl_img('footer-logo-icon.svg'); ?>" alt="" class="w-12 lg:w-[60px] h-auto">
              <div class="flex flex-col gap-1.5 lg:gap-2">
                <img src="<?php echo vbl_img('footer-logo-text.svg'); ?>" alt="Vila Baleira" class="h-[10px] lg:h-[13px] w-auto">
                <img src="<?php echo vbl_img('footer-logo-subtitle.svg'); ?>" alt="The Essence of Hospitality" class="h-[4px] lg:h-[5px] w-auto">
              </div>
          <?php endif; ?>
        </div>

        <!-- Grupo Vila Baleira Links -->
        <div class="flex flex-col gap-6">
          <p class="font-medium text-[14px] tracking-[1.4px] uppercase text-[#0d5257]">GRUPO VILA BALEIRA</p>
          <div class="grid grid-cols-2 gap-x-8 gap-y-2 font-light text-[16px] leading-[24px] text-black/80">
            <?php
            $footer_links = vbl_opt( 'footer_links', array(
            ...
            foreach ( $footer_links as $link ) :
                if ( ! empty( $link['label'] ) ) :
            ?>
                <a href="<?php echo esc_url( function_exists( 'vbl_resolve_url' ) ? vbl_resolve_url( $link['url'] ) : $link['url'] ); ?>" class="hover:text-[#bc945b] hover:underline hover:decoration-[#bc945b] hover:underline-offset-4 decoration-1 transition-all whitespace-nowrap">
        ...
        <!-- Morada -->
        <div class="flex flex-col gap-6">
          <p class="font-medium text-[14px] tracking-[1.4px] uppercase text-[#0d5257]">MORADA</p>
        ...
        <!-- Contactos -->
        <div class="flex flex-col gap-6">
          <p class="font-medium text-[14px] tracking-[1.4px] uppercase text-[#0d5257]">CONTACTOS</p>
        ...
        <!-- Siga-nos -->
        <div class="flex flex-col gap-6">
          <p class="font-medium text-[14px] tracking-[1.4px] uppercase text-[#0d5257]">SIGA-NOS</p>
      ...
      <!-- Bottom bar -->
      <div class="flex flex-col xl:flex-row items-start xl:items-center justify-between border-t border-[#bc945b]/30 pt-6 gap-6 relative">
        <div class="flex flex-wrap items-center gap-3 lg:gap-4 text-[9px] uppercase tracking-wider text-black/60 font-medium">
          ...
          ?>
              <a href="<?php echo esc_url( $resolved_url ); ?>"<?php echo $target; ?> class="hover:text-[#bc945b] hover:underline hover:decoration-[#bc945b] hover:underline-offset-4 decoration-1 transition-all"><span class="whitespace-nowrap"><?php echo esc_html( $llink['label'] ); ?></span></a><span class="text-[#bc945b] hidden lg:inline">|</span>
          <?php else : ?>
              <span class="whitespace-nowrap"><?php echo esc_html( $llink['label'] ); ?></span><span class="text-[#bc945b] hidden lg:inline">|</span>
          <?php
              endif;
          endforeach;
          ...
          if ( $nipc ) :
          ?>
              <span class="whitespace-nowrap">N.I.P.C. <?php echo esc_html( $nipc ); ?></span><span class="text-[#bc945b] hidden sm:inline">|</span>
          <?php endif;
          if ( $rnavt ) :
          ?>
        ...
        <div class="text-[9px] uppercase tracking-wider text-black/60 font-medium whitespace-nowrap">
          VILA BALEIRA &copy; <?php echo date( 'Y' ); ?> <span class="mx-2 text-[#bc945b]">|</span> <a href="https://www.sanzza.com" target="_blank" class="hover:text-[#bc945b] hover:underline hover:decoration-[#bc945b] hover:underline-offset-[3px] decoration-1 transition-all">DESIGNED BY SANZZA</a>
        </div>
      </div>
    </div>
  </footer>
	...
	<div id="barraFixaMobile" class="fixed bottom-0 left-0 right-0 z-[99999] bg-white border-t border-[#eee8e5] px-6 py-3 flex items-center justify-between shadow-[0_-4px_20px_rgba(0,0,0,0.1)] [@media(min-width:426px)]:hidden">		
    	<a href="#" class="font-body w-full flex items-center justify-between gap-4 px-6 py-4 border border-[#0d5257] text-[#0d5257] text-[14px] tracking-[1.4px] uppercase transition-all duration-300">
        	<span>Reservar</span>
	...
	<div id="backTopWrapper" class="fixed bottom-20 sm:bottom-6 xl:bottom-8 left-0 right-0 w-full pointer-events-none z-[999]">
	  <div class="max-w-[1920px] mx-auto px-6 xl:px-10 flex justify-end w-full">
	    <button id="backTop" type="button" aria-label="Voltar ao topo" class="group vbl-back-top w-10 h-10 border border-[#bc945b] flex items-center justify-center bg-white/90 backdrop-blur-sm hover:bg-[#bc945b] hover:border-[#bc945b] cursor-pointer text-[#bc945b] hover:text-white shadow-md pointer-events-auto" onclick="window.scrollTo({top:0,behavior:'smooth'})">
