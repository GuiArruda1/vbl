<?php
/**
 * Single template — Notícia individual (CPT vbl_noticia)
 * Reutiliza layout de page-modelo-noticia.php via ACF do post atual.
 *
...
<?php
// ── SECÇÃO 1: ARTIGO TOP (data + título + imagem hero) ──
$fallback_date = get_the_date( 'j \d\e F \d\e Y' );
$article_date  = vbl_field( 'vbl_noticia_article_date', false, $fallback_date );
$article_title = vbl_field( 'vbl_noticia_article_title', false, get_the_title() );
$article_hero  = vbl_field( 'vbl_noticia_article_hero_img', false, get_the_post_thumbnail_url( get_the_ID(), 'full' ) );
if ( empty( $article_hero ) ) $article_hero = vbl_img( 'modelo-noticia/hero.jpg' );
?>
...
<article class="xl:hidden px-6 md:px-10">
  <div class="pt-8 pb-6 md:pt-10 md:pb-8 flex flex-col gap-5">
    <div class="flex items-center gap-3">
      <div class="w-8 h-px bg-[#BC945B] flex-shrink-0"></div>
...
<?php
// ── SECÇÃO 2: CORPO DO ARTIGO ──
$fallback_body = apply_filters( 'the_content', get_the_content() );
$body_text_1   = vbl_field( 'vbl_noticia_body_text_1', false, $fallback_body );
$body_img      = vbl_field( 'vbl_noticia_body_img', false, '' );
$body_text_2   = vbl_field( 'vbl_noticia_body_text_2', false, '' );
?>

<!-- ====== SECÇÃO 2: CORPO DO ARTIGO — Mobile/Tablet ====== -->
<div class="xl:hidden px-6 md:px-10 mt-10 md:mt-12">
  <div class="flex flex-col gap-8">
    <div class="font-body flex flex-col gap-5 font-light text-[15px] leading-[23px]">
      <?php echo wp_kses_post( $body_text_1 ); ?>
    </div>
    <?php if ( ! empty( $body_img ) ) : ?>
    <div class="w-full overflow-hidden">
      <img src="<?php echo esc_url( $body_img ); ?>" alt="" class="w-full aspect-[800/375] object-cover">
    </div>
    <?php endif; ?>
    <?php if ( ! empty( $body_text_2 ) ) : ?>
    <div class="font-body flex flex-col gap-5 font-light text-[15px] leading-[23px]">
...
<div class="hidden xl:block max-w-[1920px] mx-auto px-[160px]" style="margin-top:80px">
  <div class="mx-auto flex flex-col gap-[40px] items-start" style="max-width:800px">
    <div class="font-body flex flex-col gap-6 font-light text-[16px] leading-[24px]">
      <?php echo wp_kses_post( $body_text_1 ); ?>
    </div>
    <?php if ( ! empty( $body_img ) ) : ?>
    <div class="w-full overflow-hidden">
...
<?php
// ── SECÇÃO 3: NAVEGAÇÃO DO ARTIGO ──
$nav_share_label = vbl_field( 'vbl_noticia_nav_share', false, 'Partilhar' );
$nav_share_url   = vbl_field( 'vbl_noticia_nav_share_url', false, get_permalink() );
$prev_post       = get_previous_post();
$next_post       = get_next_post();
$nav_prev_label  = vbl_field( 'vbl_noticia_nav_prev_label', false, $prev_post ? $prev_post->post_title : 'Anterior' );
$nav_prev_url    = vbl_field( 'vbl_noticia_nav_prev_url',   false, $prev_post ? get_permalink( $prev_post ) : '#' );
$nav_next_label  = vbl_field( 'vbl_noticia_nav_next_label', false, $next_post ? $next_post->post_title : 'Seguinte' );
$nav_next_url    = vbl_field( 'vbl_noticia_nav_next_url',   false, $next_post ? get_permalink( $next_post ) : '#' );
?>
...
<div class="xl:hidden px-6 md:px-10 mt-10 md:mt-12">
  <div class="flex items-center gap-6">
    ...
    <div class="font-body flex items-center gap-5 flex-shrink-0">
...
<div class="hidden xl:block max-w-[1920px] mx-auto px-[160px]" style="margin-top:80px">
  <div class="mx-auto flex items-center gap-[40px]" style="max-width:1280px">
    ...
    <div class="flex-1 h-px bg-[#eee8e5]"></div>
    <div class="font-body flex items-center gap-[24px] flex-shrink-0">
...
<!-- ====== Margem entre Navegação (S3) e Newsletter (S4) — Astro: mt-10 md:mt-12 xl:mt-[80px] ====== -->
